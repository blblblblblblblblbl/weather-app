...
fun CitiesGrid(
    ...
    LazyVerticalStaggeredGrid(
        ...
        items(lazyCitiesItems){item->
            item?.let { CityElement(forecast = item, refreshOnClick = {}) }
        }
    }
    ...
    Card(modifier = modifier){
        Column(
            ...
            forecast.location?.name?.let {
                Text(it, style = MaterialTheme.typography.headlineMedium, textAlign = TextAlign.Center)
            }
            ...
            forecast.current?.windKph?.let { windSpeed ->
                Row(
                    ...
                    Icon(painter = painterResource(R.drawable.wind_icon), contentDescription = "wind")
            ...
            forecast.current?.lastUpdated?.let { lastUpdated ->
                Row(
                    ...
                    IconButton(onClick = { refreshOnClick() }) {
                        Icon(Icons.Default.Refresh, contentDescription = "refresh", modifier = Modifier.size(48.dp))
                    }
                }
            }
        }
